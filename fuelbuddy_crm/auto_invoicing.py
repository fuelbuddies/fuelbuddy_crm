...
def _discount_source(so):
	"""The document whose Discount tab the discount is read from, per the "Auto
	Invoicing Discount Source" setting: the SO's Quotation (default) or the
	Opportunity behind it. All three carry the same ``custom_discount_*`` fields
	and ``custom_slab_discount`` table. Falls back Quotation -> SO so a discount
...
def _apply_quotation_discount(si, so):
	...
	if method == "Slab Discount":
		...
		slab_cap = flt(slab.threshold_value)
